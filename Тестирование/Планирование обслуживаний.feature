...
//		| 'ТестКлиент' | 'Тонкий'      | '1583' | 'File="F:\1С\Bases\DiplomWork(Netology)";' | 'Администратор' | ''       |
//	Когда Я подключаю клиент тестирования "ТестКлиент" из таблицы клиентов тестирования от имени "Менеджер" с паролем ""
...
* Затем я заполняю поле Клиент
	...
	Если таблица  "Список" не содержит строки: Тогда
			| 'Наименование'              |
			| 'Контрагент (тест-удалить)' |
		Тогда я нажимаю на кнопку с именем 'ФормаСоздать'
		Тогда открылось окно "Контрагент (создание)"
		И Я запоминаю в переменную "ИмяКонтрагента" значение "Контрагент (тест-удалить)"
...
*Затем я заполняю поле Договор
	...
	Если в таблице "Список" количество строк "равно" 0 Тогда
		...
		И я запоминаю количество строк таблицы "Список" как "КолСтрок"
		И в таблице "Список" я перехожу к первой строке
		И я запоминаю случайное число в диапазоне от "1" до "$КолСтрок$" в переменную "НомерСтроки"
		И пока выражение встроенного языка "$НомерСтроки$ > 1" истинно тогда
			И в таблице "Список" я перехожу к следующей строке
			И Я запоминаю значение выражения "$НомерСтроки$ - 1" в переменную "НомерСтроки"
		И я запоминаю значение поля с именем 'Наименование' таблицы 'Список' как 'ИмяОрганизации'
		И я нажимаю на кнопку с именем 'ФормаВыбрать'
	...
	Тогда в таблице "Список" количество строк "больше" 0
		...
		И пока выражение встроенного языка "$НомерСтроки$ > 1" истинно тогда
			И в таблице "Список" я перехожу к следующей строке
			И Я запоминаю значение выражения "$НомерСтроки$ - 1" в переменную "НомерСтроки"
		И я запоминаю значение поля с именем 'Наименование' таблицы 'Список' как 'ИмяДоговора'
	И я нажимаю на кнопку с именем 'ФормаВыбрать'
	И открылось окно "Обслуживание клиента (создание) *"
* Затем я заполняю поле Специалист
	...
	И пока выражение встроенного языка "$НомерСтроки$ > 1" истинно тогда
		И в таблице "Список" я перехожу к следующей строке
		И Я запоминаю значение выражения "$НомерСтроки$ - 1" в переменную "НомерСтроки"
	И я запоминаю значение поля с именем 'Наименование' таблицы 'Список' как 'Сотрудник'
	И я нажимаю на кнопку с именем 'ФормаВыбрать'
...
*Тогда в форме списка документа Обслуживание клиента появилась новая запись, для которой присвоен новый уникальный номер
	...
	И таблица "Список" содержит строки по шаблону:
		...
		| '$НомерДокумента$' | '$ДатаДокумента$' | '$ИмяКонтрагента$' | '15.01.2026'            | '$Сотрудник$' | 'Описание проблемы клиента (тест)' |
