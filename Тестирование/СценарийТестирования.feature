...
/////////////////НЕОБХОДИМО УКАЗАТЬ АДРЕС ИБ В КОТОРОЙ БУДЕТ ПРОИСХОДИТЬ ТЕСТИРОВАНИЕ///////////////////
	И Я запоминаю в переменную "АдресТестируемойИБ" значение 'File="E:\1С\Bases\DiplomWork(Netology)";'
////////////////////////////////////////////////////////////////////////////////////////////////////////
...
*Затем я создаю новый документ Обслуживание покупателя
	...
	*Тогда в форме списка документа Обслуживание клиента появилась новая запись, для которой присвоен новый уникальный номер
		...
		И таблица "Список" содержит строки по шаблону:
			...
			| '$НомерДокумента$' | '$ДатаДокумента$' | '$ИмяКонтрагента$' | '25.02.2026'            | '$ИмяСотрудника$' | 'Описание проблемы клиента (тест-удалить)' |
...
*Затем я закрываю созданный документ Обслуживание покупателя
	...
	*И я ищу необходимый документ
		Тогда в таблице "Список" я перехожу к строке по шаблону
			...
			| '$НомерДокумента$' | '$ДатаДокумента$' | '$ИмяКонтрагента$' | '25.02.2026'            | '$ИмяСотрудника$' |
	...
	*Затем я заполняю табличную часть по выполненным работам
		...
		И Я запоминаю значение выражения "$ЧасовРаботы1$ + $ЧасовРаботы2$ + $ЧасовРаботы3$" в переменную "СуммаЧасовРаботы"
		И Я запоминаю значение выражения "$СтоимостьРаботы1$ + $СтоимостьРаботы2$ + $СтоимостьРаботы3$" в переменную "ОбщаяСтоимостьРабот"
		И в таблице 'ВыполненныеРаботы' я завершаю редактирование строки
...
*Затем проверяю строки отчета на соответствие эталону
	И Я запоминаю значение выражения "($СтоимостьЧасаРаботы$ * $СуммаЧасовРаботы$) + $АбонентскаяПлата$" в переменную "CуммаКОплате"
	Тогда табличный документ "ОтчетТабличныйДокумент" содержит строки:
		| '*' | '$ИмяКонтрагента$' | '$ИмяДоговора$' | '$СтоимостьЧасаРаботы$' | '$СуммаЧасовРаботы$' | '$ОбщаяСтоимостьРабот$' | '$АбонентскаяПлата$' | '$CуммаКОплате$' |
